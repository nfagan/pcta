...
%
%     out = pcta.config.load() loads the current config file.
%     out = pcta.config.load( '-clear' ) clears the cache of the config
...
% cache `conf`, unless it is updated via a call to `pcta.config.save`
persistent conf;
